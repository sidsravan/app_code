...
const UserChat = ({ isVisible, setIsVisible,username,loggedInUser,viewuserid }) => {
   ...
   const getchat=async()=>{
    ...
    const chatoptions = async (value) => {
       await setLoaded(true)
        const formData = await new FormData()
        await formData.append('user_id', loggedInUser.data.session_id);
        await formData.append('to_user_id', viewuserid);
        await formData.append('status',value);
        
         await httpService('message/accept_block_delete', 'POST', formData, { signal: signal })
         .then(res => res.json())
         .then(json => {
             if (isMount) {
                        console.log("chartoptions",json);
                     //  alert(json.data)
                     getchat();
                 // setLoaded(false)
             }
    ...
    return (
        ...
        <Modal
            ...
            <Footer style={{ paddingHorizontal: 5, marginBottom: Platform.OS === 'ios' ? 30 : 2, backgroundColor: '#fff', minHeight: 45, height: height, maxHeight: 120 }}>
                ...
                <View style={{flexDirection:'row',justifyContent:'space-evenly',height:60,backgroundColor:"#ffffff",alignItems:'center'}}>
                   <TouchableOpacity onPress={()=>chatoptions(2)}>
                    <Text style={{fontSize:16,fontWeight:'900',color:"#808080"}}>Block</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={()=>chatoptions(3)}>
                    <Text style={{fontSize:16,fontWeight:'900',color:"#808080"}}>Delete</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={()=>chatoptions(1)}>
                    <Text style={{fontSize:16,fontWeight:'900',color:"#808080"}}>Accept</Text>
                    </TouchableOpacity>
                </View>
